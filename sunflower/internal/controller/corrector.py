...
import pylab as pl
from sunflower.internal.util.recorder import Recorder, HA, DEC
...
class Corrector(object):
    ...
    def update(self):
        ...
        dec_list = self.recorder.readData(kind=DEC)
        ...
        try:
            self.offsetMap['ha'], self.offsetMap['haOffset'] = self._fit(x=has, y=haOffsets, scale=[-180, 180, 3600])
            self.show(x=self.offsetMap['ha'], y=self.offsetMap['haOffset'], title='ha-haOffset')
        except ValueError as e:
            print("data of ha is too less")
        try:
            self.offsetMap['dec'], self.offsetMap['decOffset'] = self._fit(x=decs, y=decOffsets, scale=[-90, 90, 1800])
            self.show(self.offsetMap['dec'], self.offsetMap['decOffset'], 'dec-decOffset')
        except ValueError as e:
            print("data of dec is too less")
